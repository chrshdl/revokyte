...
class LapTimeWidget(Widget):
    ...
    def update(self, bus: VehicleBus, dt: float):
        ...
        if not (frame.last_lap_time == 0 or frame.last_lap_time is None):
            last_lap_time = float(frame.last_lap_time * 1e-3)
            self.set_value(last_lap_time)
