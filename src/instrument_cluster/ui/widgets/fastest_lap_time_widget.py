...
class FastestLapTimeWidget(Widget):
    """
    ...
    def update(self, bus: VehicleBus, dt: float):
        ...
        if not (frame.best_lap_time == 0 or frame.best_lap_time is None):
            best_lap_time = float(frame.best_lap_time * 1e-3)
            self.set_value(best_lap_time)
