...
`TelemetryMode.DEMO` plays back a recorded session from `assets/`. `TelemetryMode.UDP` listens on `udp_host:udp_port` for live telemetry from whichever feed is installed — the runtime mode is UDP for *every* game, so there is no per-game `TelemetryMode`. The settings dropdown offers Demo plus one entry per `FeedDescriptor` in `addons/feeds.py`; selecting a feed routes through `EnterIPState` → `InstallState` (which installs that feed's signed tarball and writes its env file), then sets `telemetry_mode=udp` and records the chosen feed id in `config.telemetry_feed` (an opaque key used only to show the current dropdown selection). Switch via `ConfigManager.set_telemetry_mode()` / `set_telemetry_feed()` or through the settings UI.
