...
### Engine Simulation (shift lights + fuel)

`core/engine_sim/` is a self-contained crank-angle-resolved single-zone ICE model: slider-crank kinematics, parametric cam lobes with compressible orifice valve flow (volumetric efficiency *emerges* from cam geometry — nothing prescribes it), quasi-static MAP solve per operating point with turbo boost + fitted wastegate compensation, Wiebe combustion anchored on a CA50 timing target (the MBT proxy — no advance search anywhere), Woschni heat transfer, Chen–Flynn friction. Rotaries/two-strokes run through the same integrator with equivalent parameters. It **never runs on the frame loop**: `EngineSimService` (module singleton, `service.py`) bakes an RPM×throttle torque+fuel map (~2.5 KB) on a background daemon thread at car change — latest-request-wins, mid-bake cancellation and 2 ms sleeps via the integrator's pace hook (every ~90 crank steps) so the 60 fps loop keeps its core. Consumers `request(car_id)` on car change, `poll(car_id)` per frame.

- **Shift lights**: `peripherals/shift_lights.py` builds the heuristic `EngineModel` from the first frame (LEDs never wait for a bake), then swaps in the calibrated curve via `ShiftLightController.install_engine_map(MappedEngineModel(...))` when the bake lands — the existing ratios-changed branch rebuilds `ShiftPointCalculator` from it. A telemetry rev-limit above the baked axis triggers an extended re-bake (`ensure_rpm`); the model extrapolates with the heuristic's over-rev decay meanwhile.
- **Fuel**: `signals/fuel_flow.py::FuelFlowObserver` (composed into `FuelSignal`) integrates the map's g/s at the live (rpm, throttle) between measured `gas_level` anchors and learns a units-per-gram scale from real drops (clamped EMA) — the game's fuel logic stays ground truth; the model only fills the space between its samples, and publishes `fuel_rate`. Without a map (ACC, demo's `car_id=-1`, missing artifact) it is inert and `FuelSignal` behaves exactly as before.
- **Calibration is offline**: `tools/engine_sim/fit_cars.py` fits every cars.json entry (+ `"default"` = the `CarLibrary` fallback profile) to its DB peaks — archetype templates (`archetypes.py`) + per-car pins (`tools/engine_sim/overrides.json`: rotaries, diesels, the kart, EV bypasses, turbo exotics the 5-scalar shape rules can't identify), staged fitter (`calibrate.py`: displacement → ram-tuning torque-peak placement → *joint* breathing+friction iteration (sequential stages spiral) → bounded ±5% correction blend), writing the checked-in `db/engine_params.json`. Regenerate whenever cars.json, the templates or the physics change — `test_calibration_artifact.py` gates it. `fit: failed` (e.g. SRT Tomahawk X VGT — clamps say no) and `ev_bypass` keep the heuristic at runtime. Validation accepts a mild power rise to redline: the DB's redline is `max_power_rpm + 1000` everywhere, so "peaks at rated rpm" vs "still pulling at redline" is indistinguishable in the source data.
- **Pedal units**: GT7 transmits throttle/brake as raw 0–255 bytes; the direct readers normalize to 0..1 (schema range) and throttle consumers additionally guard with `telemetry/units.py::ThrottleNormalizer` (session-latched raw-byte detection) because a separately-released NDJSON proxy feed may still emit raw bytes — the proper fix there is a feed release.
- **Tools**: `tools/engine_sim/dyno.py` (model vs heuristic vs DB peak markers, part-throttle, fuel), `bench_bake.py` (bake wall time; soft budget 2 s on Pi 4), `fit_cars.py --report` (failure triage).
- **Empirical validation**: the dashboard's **Dyno** footer button (right of Setup) pushes `AccelLogState` — the accel-run logger. `core/engine_sim/accel_recorder.py::AccelRunRecorder` (UI-free, fed from the bus each tick) auto-detects full-throttle pulls (pedal ≥ 0.95 in a forward gear; ends on lift, upshift, or limiter bounce), gates quality (≥ 1.5 s, ≥ 1500 rpm span), and saves accepted runs as JSON under `<config dir>/accel_runs/car_<id>/` (per-sample rpm/speed/throttle + wheel ground speeds for offline slip filtering). Demo's `car_id=-1` keeps it idle. `tools/engine_sim/analyze_accel.py` compares the measured torque *shape* (accel-derived, normalized — shift points depend only on shape, so no mass/drag constants needed) against the model and the heuristic.
